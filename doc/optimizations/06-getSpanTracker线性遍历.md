...
想象你有一个**档案柜**（`trackerArray_`），里面存着很多**档案页**（`SpanTracker`）。
*   **以前的做法（O(n)）：** 档案是乱序放进去的。你手里拿着一个地址（`blockAddr`），必须从第一页翻到最后一页，挨个对比，直到找到对应的档案。如果档案有 1000 张，你平均要翻 500 次。
*   **理想的做法：** 如果档案按地址从小到大排好序，你可以用**二分查找**（像查字典一样，先翻一半，再翻一半），1000 张档案只需要查 10 次。

**难点：** 档案是动态增加的。如果每次加一张新档案都重新排一次序，那写操作就太慢了。

---

### 2. 精妙设计：懒排序（Lazy Sorting）

为了平衡“查找快”和“插入快”，代码把档案柜分成了两部分：

1.  **已排序区（Sorted Part）：** 索引从 `0` 到 `sortedCount_`。这部分是用**二分查找**，极快。
2.  **未排序区（Unsorted Tail）：** 索引从 `sortedCount_` 到 `spanCount_`。这部分依然用**线性扫描**。

**这里的权衡（Trade-off）：**
代码规定：未排序的档案最多只能有 **64 个**（`SORT_THRESHOLD`）。
*   当你找东西时：先二分查找大部队，没找到再扫一眼这 64 个。
*   当你插入时：直接扔到末尾，不用排队，速度极快。
*   当未排序区达到 65 个时：触发一次“大排队”（`ensureSorted`），把所有档案重新排序，然后全部归入“已排序区”。

---

### 3. 三个执行步骤：代码到底干了啥？

#### 第一步：查找（getSpanTracker）
它现在分两步走：
1.  **二分查找（快）：** 在 `[0, sortedCount_)` 范围内查找。因为这部分是有序的，查找复杂度从 $1000$ 降到了 $\log(1000) \approx 10$。
2.  **兜底扫描（短）：** 在剩下的 `[sortedCount_, total)` 查找。因为这部分最多只有 64 个，扫描速度非常稳定且很快。

#### 第二步：触发（fetchRange 分支 A）
每当中央缓存（CentralCache）申请了一个新的 Span，它就往 `trackerArray_` 后面塞一个新的指针。
塞完后检查：**“没排序的档案是不是超过 64 个了？”**
如果是，就调用 `ensureSorted()`。

#### 第三步：重排（ensureSorted — 最关键的无锁思想）
为了保证在排序时，别的线程还能继续查找，它用了**“原子替换”**的技巧：
1.  **加锁：** 防止多个线程同时去重排（`std::lock_guard`）。
2.  **建新副本：** 申请一个新的指针数组（`newArray`），把旧的拷贝过来。
3.  **原地排序：** 对这个**新数组**进行 `std::sort`。
4.  **原子交换：** 把档案柜的“索引表”指针（`trackerArray_`）指向这个新数组。
    *   *注：* 正在查找的线程还在看旧表，没关系，旧表也是有效的。新的查找请求会看到新表。
5.  **更新标记：** 把 `sortedCount_` 设为当前总数，表示现在全部有序了。